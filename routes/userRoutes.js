...
function checkFileType(file, cb) {
  const filetypes =
    /jpeg|jpg|png|gif|image\/jpeg|image\/png|application\/octet-stream/;
  ...
  console.log("File yang diunggah:", {
    ...
    isMimetypeMatch: mimetype,
    isExtnameMatch: extname,
  });
...
function getLogoUrl(brand, model) {
  ...
  if (brandLower === "honda") {
    ...
    if (modelLower.includes("nex ii") || modelLower.includes("nex 2"))
      logoUrl = "/logos/suzuki_nexii.png";
  }
  return logoUrl;
}
...
router.post("/register", async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({
        message: "Data pengguna dan kendaraan utama tidak boleh kosong.",
      });
  }
  ...
  try {
    ...
    if (
      receivedInitialServices &&
      Array.isArray(receivedInitialServices) &&
      receivedInitialServices.length > 0
    ) {
      for (const service of receivedInitialServices) {
        if (
          service.service_type &&
          service.odometer_at_service !== undefined &&
    ...
    if (
      ...
      return res
        .status(400)
        .json({
          message: "Validasi gagal atau data duplikat.",
          errors: messages,
        });
    }
    res
      .status(500)
      .json({ message: "Error server saat registrasi.", error: error.message });
  }
});
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error server saat login.", error: error.message });
  }
});
...
router.get("/profile", authMiddleware, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error server saat mengambil profil.",
        error: error.message,
      });
  }
});
...
router.put("/profile/update", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error server saat update profil.",
        error: error.message,
      });
  }
});
...
router.post(
  ...
  async (req, res) => {
    // Middleware untuk menangani error dari Multer (seperti file terlalu besar atau tipe salah)
    ...
    try {
      ...
      // Pastikan path menggunakan forward slash untuk konsistensi
      ...
      // Kirim pesan error yang lebih spesifik ke client
      res
        .status(500)
        .json({
  ...
  (error, req, res, next) => {
    ...
    res.status(400).json({ message: `Error dari Multer: ${error.message}` });
  }
);
